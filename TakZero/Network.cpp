...
void Network::benchmark(Board * state, int iterations) {
	...
	printf("%5d evaluations in %5.2f seconds -> %d n/s\n",
...
Network::Netresult Network::get_scored_moves(
	Board * state, Ensemble ensemble, int rotation) {
	Netresult result;
	if (state->SIZE != 5) {
		return result;
	}
...
void Network::show_heatmap(Board * state, Netresult& result, bool topmoves) {
	auto moves = result.first;
	std::vector<std::string> display_map;
	std::string line;

	for (unsigned int y = 0; y < state->SIZE; y++) {
...
void Network::gather_features(Board * state, NNPlanes & planes) {
	planes.resize(8);
	constexpr size_t our_offset = 0;
	constexpr size_t their_offset = 8;
	BoardPlane& black_to_move = planes[6];
	BoardPlane& white_to_move = planes[7];
