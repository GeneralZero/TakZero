...
Random::Random(std::uint64_t seed) {
	if (seed == 0) {
		...
		seedrandom(ConfigStore::get().ints.at("cfg_rng_seed") ^ (std::uint64_t)thread_id);
	}
	else {
		seedrandom(seed);
	}
...
std::uint16_t Random::randuint16(const std::uint16_t max) {
	return ((gen() >> 48) * max) >> 16;
}

std::uint32_t Random::randuint32(const std::uint32_t max) {
