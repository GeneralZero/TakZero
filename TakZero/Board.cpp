...
std::array< uint8_t, 5 * 5 * 32>* Board::CreateFastBoard() {
	auto new_board = new std::array< uint8_t, 5 * 5 * 32>{0};
	for (uint8_t i = 0; i < this->SIZE * this->SIZE; i++)
	{
		uint8_t j = 0;
		uint8_t to_put = 0;
...
void Board::PlayMove(Play move)
{
	...
	//Update prevboards
	//auto new_board = this->CreateFastBoard();
...
void Board::WallCrush(Location current_square, std::vector<Piece> pop_array) {
	...
	if ((pop_array[0] & Capstone) == Capstone && (cell->back() & Standing ) == Standing ) {
		//Capstone Crush
	...
	if (cell->size() == 0) {
		...
		if (this->move_number >= this->SIZE*this->SIZE -1) {
			...
			for (uint8_t i = 0; i < this->board.size(); i++)
			{
				...
				else if ((this->board[i].back() & White) == White) {
					white_top_count++;
				}
				else {
					black_top_count++;
...
void Board::WinnerPlace(PieceType piece, Location location)
{
	...
	if (piece != Standing) {
		...
		while (!to_check.empty()){
			...
			if ( (this->white_turn && cell->size() != 0 && ((cell->back() & Black) == White )) || \
				(!this->white_turn && cell->size() != 0 && ((cell->back() & Black) == Black))) {
...
void Board::WinnerMove(std::vector<Location> locations)
{
	...
	for each (Location location in locations)
	{
		...
		while (!to_check.empty()) {
			...
			//Check turn and matching top
			if ((this->white_turn && cell->size() != 0 && ((cell->back() & White) == White)) || \
				(!this->white_turn && cell->size() != 0 && ((cell->back() & Black) == Black))) {
...
uint16_t Board::GetPlacementIndex(Play move) {
	...
	if ((move.piece & Standing) == Standing) {
	...
	if ( this->move_number <= 1 ) {
		for (uint8_t i = 0; i < this->board.size(); i++)
		{
			if (this->board[i].size() == 0 ) {
				...
				temp.location.y = floor(i / this->SIZE);
		...
		for (uint8_t i = 0; i < this->board.size(); i++)
		{
			if (!(this->board[i].size() == 0)) {
				...
				std::vector<Piece>* cell = &this->board[i];

				//Get White tops
				if (this->white_turn) {
					if ((cell->back() & White) == White) {
						uint8_t distance = 0;
						uint8_t s_distance = 0;

						uint8_t to_move = std::min(this->SIZE, (uint8_t)cell->size());

						//Down
						for (size_t j = (i + this->SIZE); j < this->SIZE * this->SIZE; j += this->SIZE)
						{
							if (this->board[j].size() == 0 || ((this->board[j].back() & Capstone) == 0)) {
								distance++;
							}
							else if (cap && (this->board[j].back() & Standing) == Standing) {
					...
					if ((cell->back() & Black) == Black) {
						...
						s_distance = 0;
						//Right
						for (size_t j = (i + 1); j > (i + (i % this->SIZE) + this->SIZE - 1); j++)
						{
							if (this->board[j].size() == 0 || ((this->board[j].back() & Capstone) == 0)) {
								distance++;
							}
...
Play Board::GetMoveFromIndex(uint16_t move)
{
	...
	if (move >= 1500 ) {
		...
		ret.piece = (PieceType)(offset - (i * 3));
	}
	else {
		ret.type = MoveStack;
		ret.index = move;
