...
class TakZeroNetwork():
	...
	def __init__(self):
		self.input_layer = Input(shape=(8, 5, 5, 32))
	...
	def training_files_generator(self, file_names):
		...
		for file_name in file_names:
			...
			with h5py.File(os.path.join(os.getcwd(), file_name), 'r') as hf:
				x_train   = hf["x_train"][:]
...
if __name__ == '__main__':
	...
	training_files = [filename for filename in os.listdir(os.path.join(os.getcwd())) if filename.startswith("Game_")]
